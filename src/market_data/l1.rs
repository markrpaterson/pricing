...
/// market, or for pricing a Fixed Income instrument which tends to be priced this way.
///
/// # Generic Parameters
///
/// * `P` - The price type that should be used.
pub struct L1MarketData<P>
where
    ...
    P: Copy + PartialOrd + Add<Output = P> + Div<Output = P> + From<i32>,
{
    ...
    ///
    /// let market_data = L1MarketData::<i32>::new();
    ///
    ...
    /// use pricing::market_data::L1MarketData;
    ///
    /// let market_data = L1MarketData::new_with_price(Some(10), Some(20));
    ///
    ...
    /// use pricing::market_data::L1MarketData;
    ///
    /// let mut market_data = L1MarketData::new_with_price(Some(10), Some(20));
    ///
    /// assert_eq!(*market_data.get_bid(), Some(10));
    ///
    ...
    pub fn clear(&mut self) {
        if self.price.get_bid().is_some() || self.price.get_offer().is_some() {
            self.price = BidOffer::new();
            self.publish_to_subscribers();
        }
    }

    /// Returns the price for the size passed in, which is always the current pricing.
    ///
    /// # Returns
    ///
    ...
    /// }
    ///
    /// let mut test = L1MarketData::new_with_price(Some(10), Some(10));
...
impl<P> Default for L1MarketData<P>
where
    P: Copy + PartialOrd + Add<Output = P> + Div<Output = P> + From<i32>,
{
    fn default() -> Self {
...
/// * `P` - The price type that should be used.
pub struct L1MarketDataWithMax<P, A>
where
    P: Copy + PartialOrd + Add<Output = P> + Div<Output = P> + From<i32>,
...
impl<P, A> L1MarketDataWithMax<P, A>
where
    P: Copy + PartialOrd + Add<Output = P> + Div<Output = P> + From<i32>,
    A: Copy + PartialOrd + Add<Output = A> + Div<Output = A> + From<i32>,
{
    /// Use the new function to create a new L1MarketDataWithMax with no pricing.
    ///
    /// # Example
    ///
    /// ```
    /// use pricing::market_data::L1MarketDataWithMax;
    ///
    /// let market_data = L1MarketDataWithMax::<i32, i32>::new();
    ///
    /// assert_eq!(*market_data.get_bid(), None);
    /// assert_eq!(*market_data.get_offer(), None);
    ...
    /// Use the new function to create a new L1MarketDataWithMax with a bid and offer price.
    ///
    /// # Parameters
    ///
    /// * `bid` - The bid price, a value of None means no price available
    ...
    /// use pricing::market_data::L1MarketDataWithMax;
    ///
    /// let market_data = L1MarketDataWithMax::<_, i32>::new_with_price(Some(10), Some(20));
    ///
    /// assert_eq!(*market_data.get_bid(), Some(10));
    /// assert_eq!(*market_data.get_offer(), Some(20));
    /// assert_eq!(*market_data.get_max_bid(), None);
    ...
    /// ```
    pub fn new_with_price(bid: Option<P>, offer: Option<P>) -> Self {
        L1MarketDataWithMax::new_with_max(bid, offer, None, None)
    }

    /// Use the new function to create a new L1MarketDataWithMax with a bid and offer price as well as maximum sizes.
    ///
    /// # Parameters
    ///
    /// * `bid` - The bid price, a value of None means no price available
    ...
    /// use pricing::market_data::L1MarketDataWithMax;
    ///
    /// let market_data = L1MarketDataWithMax::new_with_max(Some(10), Some(20), Some(100), Some(101));
    ///
    /// assert_eq!(*market_data.get_bid(), Some(10));
    /// assert_eq!(*market_data.get_offer(), Some(20));
    ...
    /// use pricing::market_data::L1MarketDataWithMax;
    ///
    /// let market_data = L1MarketDataWithMax::new_with_max(Some(10), Some(20), Some(90), Some(110));
    ///
    ...
    /// use pricing::market_data::L1MarketDataWithMax;
    ///
    /// let mut market_data = L1MarketDataWithMax::<_, i32>::new_with_price(Some(10), Some(20));
    ///
    /// assert_eq!(*market_data.get_bid(), Some(10));
    ///
    /// market_data.update_bid(Some(12));
    ...
    /// use pricing::market_data::L1MarketDataWithMax;
    ///
    /// let mut market_data = L1MarketDataWithMax::<_, i32>::new_with_max(Some(10), Some(20), Some(40), Some(50));
    ///
    ...
    ///
    /// ```
    /// use pricing::market_data::{BidOffer, L1MarketDataWithMax};
    ///
    /// let mut market_data = L1MarketDataWithMax::<_, i32>::new_with_price(Some(10), Some(20));
    ///
    /// assert_eq!(*market_data.get_bid(), Some(10));
    ...
    /// use pricing::market_data::L1MarketDataWithMax;
    ///
    /// let mut market_data = L1MarketDataWithMax::new_with_max(Some(10), Some(20), Some(40), Some(50));
    ///
    /// assert_eq!(*market_data.get_bid(), Some(10));
    /// assert_eq!(*market_data.get_offer(), Some(20));
    /// assert_eq!(*market_data.get_max_bid(), Some(40));
    ...
    /// use std::cell::RefCell;
    /// use std::rc::Rc;
    /// use pricing::market_data::{L1MarketCallback, L1MarketDataWithMax};
    ///
    /// struct TestCallback {
    ///     called: RefCell<bool>,
    /// }
    ...
    /// }
    ///
    /// let mut test = L1MarketDataWithMax::<_, i32>::new_with_price(Some(10), Some(10));
    /// let callback = Rc::new(TestCallback::new());
    /// test.subscribe(callback.clone());
    ///
    /// test.update_bid(Some(10));
...
impl<P, A> Default for L1MarketDataWithMax<P, A>
where
    P: Copy + PartialOrd + Add<Output = P> + Div<Output = P> + From<i32>,
    A: Copy + PartialOrd + Add<Output = A> + Div<Output = A> + From<i32>,
{
...
mod tests {
    ...
    fn max_not_applied_when_not_set() {
        let test = L1MarketDataWithMax::new_with_max(Some(10), Some(10), Some(10), None);
        ...
        let test = L1MarketDataWithMax::new_with_max(Some(10), Some(10), None, Some(10));
    ...
    #[test]
    fn update_individual_triggers_subscriptions_with_max() {
        let mut test = L1MarketDataWithMax::new_with_max(Some(10), Some(10), Some(60), Some(70));
    ...
    #[test]
    fn update_multiple_triggers_subscriptions_with_max() {
        let mut test = L1MarketDataWithMax::new_with_max(Some(10), Some(10), Some(60), Some(70));
    ...
    fn clear_triggers_subscriptions() {
        ...
        test.update(Some(10), None);
        callback.reset();
        test.clear();
        assert!(callback.is_called());

        test.update(None, Some(10));
        callback.reset();
        test.clear();
        assert!(callback.is_called());
    }
    ...
    #[test]
    fn clear_triggers_subscriptions_with_max() {
        let mut test = L1MarketDataWithMax::new();
